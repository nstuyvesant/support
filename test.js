...
let testResults = {
    ...
    isp: '',
    timezone: '',
    dataCenters: [ // Retains results for easy email or download, use -1 as unknown value
        {
            name: 'Boston',
            code: 'bos',
...
$(document).ready(function() {
    ...
    $.getJSON('https://support.perfecto.io/php/ip-info.php', function(response) {
        ...
        // Selectively assign vs. testResults = response;
        testResults.ip = response.ip;
        testResults.proxy = response.proxy;
        testResults.location = location;
        testResults.isp = response.isp;
        testResults.timezone = response.timezone;
    });
...
var streamTesting = 'none'; // possible values: none | init | active | error | abort (if line 118)
var state = [];
...
function speedTestUpdate() {
    if (testTypeRunning === 'Network') // speedtest is active
        ...
        if (player.getState() === 'error') {
            ...
            let lastStreamingScore = Math.round(100 - (100 / (streamingTime / 100) * state['buffering']));
