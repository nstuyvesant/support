...
// TODO: Drop unused commented code
...
$('body').on('speedTestComplete', function () {
  ...
  if (playerEnabled) {
    // startTestStream() // no longer need to setup the test stream (using /vod/sample.mp4)
    player.setConfig({ autostart: true }) // Don't need to invoke player.play() as we advance through playlist
    // TODO: Autostart will cause first stream to play so we might need to prevent player.playlistItem() from executing for first streamType (or disable autostart and use explicit)
...
function testNextStreamer () {
  ...
  if (selectedStreamType > 0) {
    ...
    if (selectedStreamType === streamTypes.length) { // no more stream types to test
      ...
      // stopStream() // no longer needed for /vod/sample.mp4
      return
    }
  }
