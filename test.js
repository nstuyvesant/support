...
function testNextStreamer () {
  ...
  if (selectedStreamType > 0) {
    ...
    let quality = elapsedTime !== 0 ? Math.round(played * 100 / elapsedTime) : -1 // prevent divide by zero
    ...
    // TODO: scale score so 0 to 49 is bad, 50 to 75 is meh, 76 to 100 is good (and drop percentage sign)
    ...
    if (selectedStreamType === streamTypes.length) { // no more stream types to test
      ...
      // Else done with streaming tests until next data center test (called from stopStream())
      stopStream()
      return
    }
  }
...
$(document).ready(function () {
  ...
  player.on('firstFrame', function () {
    let qoe = player.qoe()
    let firstFrame = (qoe.firstFrame / 1000).toFixed(2)
